...
The existing Logic App still accepts the built-in Request trigger, records the event in run history, and returns 202 before any email action. The named `gmail-kpmg-notify` API connection was created by `infra/gmail-connection.bicep`, but Azure currently reports its status as Error while OAuth is unconfigured. Gmail delivery is **NOT AUTHORIZED / NOT VERIFIED**; changing the live workflow before authorization risks invalidating the working ADF notification endpoint. No email-delivery claim or inbox screenshot should be used. Priya must open Azure portal → resource group → API connections → gmail-kpmg-notify → Edit API connection → Authorize → Save. Once the connection is Connected, add Gmail Send email (V2) after Response, test one event, and verify both workflow run and inbox. For a consumer Gmail account, Microsoft's Google policy permits the built-in Request trigger but restricts the separate HTTP connector/action elsewhere in the workflow.
