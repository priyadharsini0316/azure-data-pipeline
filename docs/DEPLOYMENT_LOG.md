...
Priya authorized the named `gmail-kpmg-notify` connection, and Azure reports **Connected**. `infra/gmail-workflow-extension.bicep` deployed Gmail Send email (V2) after the existing 202 Response, preserving the ADF-facing Request/Compose/Response path. The first manual `TEST_EMAIL` POST received HTTP 202, but Logic App run `08584120258021519282617111833CU16` failed at `Send_Gmail_Alert` with HTTP 403, **Request had insufficient authentication scopes**. Priya re-authorized Gmail; retest run `08584120255427365822838551254CU09` then **Succeeded**, with Compose, Response, and Send_Gmail_Alert all Succeeded. Priya separately confirmed receipt in Gmail. The 202 response keeps ADF decoupled from email send outcomes. A sanitized inbox screenshot is still needed for submission evidence.
